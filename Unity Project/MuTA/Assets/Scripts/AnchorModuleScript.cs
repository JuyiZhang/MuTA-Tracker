...
public class AnchorModuleScript : MonoBehaviour
{
    ...
    {
        currentDebugger.AddDebugMessage("AnchorModuleScript.StartAzureSession()");

        // Notify AnchorFeedbackScript
        OnStartASASession?.Invoke();

        currentDebugger.AddDebugMessage("Starting Azure session... please wait...");
    ...
    public async void StopAzureSession()
    {
        ...
        currentDebugger.AddDebugMessage("Stopping Azure session... please wait...");
        ...
        currentDebugger.AddDebugMessage("Azure session stopped successfully");
    }
    ...
    public async void CreateAzureAnchor(GameObject theObject)
    {
        ...
        {
            currentDebugger.AddDebugMessage("Didn't get the local anchor...");
            return;
        }
        else
        {
            currentDebugger.AddDebugMessage("Local anchor created");
        }
        ...
        while (!cloudManager.IsReadyForCreate)
        {
            ...
            QueueOnUpdate(new Action(() => currentDebugger.AddDebugMessage($"Move your device to capture more environment data: {createProgress:0%}")));
        }
        ...
        try
        {
            currentDebugger.AddDebugMessage("Creating Azure anchor... please wait...");
            ...
            if (success)
            {
                currentDebugger.AddDebugMessage($"Azure anchor with ID '{currentCloudAnchor.Identifier}' created successfully");

                // Notify AnchorFeedbackScript
                OnCreateAnchorSucceeded?.Invoke();

                // Update the current Azure anchor ID
                currentDebugger.AddDebugMessage($"Current Azure anchor ID updated to '{currentCloudAnchor.Identifier}'");
                currentAzureAnchorID = currentCloudAnchor.Identifier;
            }
            else
            {
                currentDebugger.AddDebugMessage($"Failed to save cloud anchor with ID '{currentAzureAnchorID}' to Azure");
        ...
        {
            currentDebugger.AddDebugMessage(ex.ToString());
        }
    }
    ...
    public void RemoveLocalAnchor(GameObject theObject)
    {
        ...
        {
            currentDebugger.AddDebugMessage("Local anchor deleted succesfully");
        }
        else
        {
            currentDebugger.AddDebugMessage("Attempt to delete local anchor failed");
        }
    }
    ...
    public void FindAzureAnchor(string id = "")
    {
        ...
        {
            currentDebugger.AddDebugMessage("Current Azure anchor ID is empty");
            return;
        }

        anchorLocateCriteria.Identifiers = anchorsToFind.ToArray();
        currentDebugger.AddDebugMessage($"Anchor locate criteria configured to look for Azure anchor with ID '{currentAzureAnchorID}'");

        // Start watching for Anchors
        if ((cloudManager != null) && (cloudManager.Session != null))
        {
            currentWatcher = cloudManager.Session.CreateWatcher(anchorLocateCriteria);
            currentDebugger.AddDebugMessage("Watcher created");
            currentDebugger.AddDebugMessage("Looking for Azure anchor... please wait...");
        }
        else
        {
            currentDebugger.AddDebugMessage("Attempt to create watcher failed, no session exists");
            currentWatcher = null;
        }
    }
    ...
    public async void DeleteAzureAnchor()
    {
        ...
        currentDebugger.AddDebugMessage("Azure anchor deleted successfully");
    }
    ...
    public void SaveAzureAnchorIdToDisk()
    {
        ...
        currentDebugger.AddDebugMessage($"Current Azure anchor ID '{currentAzureAnchorID}' successfully saved to path '{filePath}'");
    }
    ...
    /*public void ShareAzureAnchorIdToNetwork()
    {
        ...
        try
        {
            ...
            currentDebugger.AddDebugMessage($"Connecting to network client '{client}'... please wait...");
            ...
            currentDebugger.AddDebugMessage("Uploading file... please wait...");

            string json = httpResponse.Content.ToString();
        }
        catch (Exception ex)
        {
            currentDebugger.AddDebugMessage(string.Format("Exception: {0}", ex.Message));
            throw;
        }

        currentDebugger.AddDebugMessage($"Current Azure anchor ID '{currentAzureAnchorID}' shared successfully");
    }
    ...
    private void CloudManager_AnchorLocated(object sender, AnchorLocatedEventArgs args)
    {
        ...
        if (args.Status == LocateAnchorStatus.Located || args.Status == LocateAnchorStatus.AlreadyTracked)
        {
            ...
            QueueOnUpdate(() =>
            {
                currentDebugger.AddDebugMessage($"Azure anchor located successfully");
                ...
                {
                    currentDebugger.AddDebugMessage("Local anchor position successfully set to Azure anchor position");
                    transform.position = currentCloudAnchor.GetPose().position;
                    transform.rotation = currentCloudAnchor.GetPose().rotation;
                    //gameObject.GetComponent<UnityEngine.XR.WSA.WorldAnchor>().SetNativeSpatialAnchorPtr(currentCloudAnchor.LocalAnchor);
                }

                currentDebugger.AddDebugMessage($"Current pose of the anchor is: {currentCloudAnchor.GetPose().position}");

#elif UNITY_ANDROID || UNITY_IOS
                Pose anchorPose = Pose.identity;
                anchorPose = currentCloudAnchor.GetPose();

                currentDebugger.AddDebugMessage($"Setting object to anchor pose with position '{anchorPose.position}' and rotation '{anchorPose.rotation}'");
        ...
        {
            QueueOnUpdate(new Action(() => currentDebugger.AddDebugMessage($"Attempt to locate Anchor with ID '{args.Identifier}' failed, locate anchor status was not 'Located' but '{args.Status}'")));
        }
    }
    #endregion
    ...
    IEnumerator GetSharedAzureAnchorIDCoroutine(string sharingPin)
    {
        ...
        currentDebugger.AddDebugMessage($"Looking for url '{url}'... please wait...");
        ...
        using (UnityWebRequest www = UnityWebRequest.Get(url))
        {
            ...
            {
                currentDebugger.AddDebugMessage(www.error);
            }
            else
            {
                currentDebugger.AddDebugMessage("Downloading... please wait...");
                ...
                currentDebugger.AddDebugMessage($"Current Azure anchor ID successfully updated with shared Azure anchor ID '{currentAzureAnchorID}' url");
