...
namespace winrt::HL2UnityPlugin::implementation
{
            ...
            while (pHL2ResearchMode->m_longDepthSensorLoopStarted)
            {
                ...
                for (UINT i = 0; i < resolution.Height; i++)
                {
                    for (UINT j = 0; j < resolution.Width; j++)
                    {
                        ...
                        depthData.push_back((float)depth);

                        if (pHL2ResearchMode->m_reconstructLongThrowPointCloud)
                        {
                            // back-project point cloud within Roi [With ALL VALUE]
                            //if (i > pHL2ResearchMode->depthCamRoi.kRowLower * resolution.Height && i < pHL2ResearchMode->depthCamRoi.kRowUpper * resolution.Height &&
                            //    j > pHL2ResearchMode->depthCamRoi.kColLower * resolution.Width && j < pHL2ResearchMode->depthCamRoi.kColUpper * resolution.Width &&
                            //    depth > 200)
                            //{
                ...
                // save data
                {
                    ...
                    if (!pHL2ResearchMode->m_longDepthData) {
                        OutputDebugString(L"Create Space for long throw depth data...\n");
                        pHL2ResearchMode->m_longDepthData = new float[outBufferCount];
                    }
                    memcpy(pHL2ResearchMode->m_longDepthData, depthData.data(), depthData.size() * sizeof(float));
                    pHL2ResearchMode->m_longDepthDataLength = depthData.size();
                ...
                pHL2ResearchMode->m_longDepthDataUpdated = true;
    ...
    void HL2ResearchMode::StopAllSensorDevice()
    {
        ...
        if (m_longDepthData)
        {
            m_longDepthDataLength = 0;
            delete[] m_longDepthData;
            m_longDepthData = nullptr;
        }
    ...
    com_array<float> HL2ResearchMode::GetLongDepthDataBuffer() {
        std::lock_guard<std::mutex> l(mu);
        if (m_longDepthDataLength == 0)
        {
            return com_array<float>();
        }
        com_array<float> tempBuffer = com_array<float>(std::move_iterator(m_longDepthData), std::move_iterator(m_longDepthData + m_longDepthDataLength));
        m_longDepthDataUpdated = false;
        return tempBuffer;
    }
